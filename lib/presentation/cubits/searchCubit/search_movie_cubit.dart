...
import 'package:movies/presentation/cubits/searchCubit/search_movie_state.dart';

import '../../../domain/model/movieModel/movie_model.dart';


class SearchMoviesCubit extends Cubit<SearchMoviesState> {
  SearchMoviesCubit(this.moviesRepo) : super(SearchMoviesInitial());
  ...
  void getMovies({required String title}) async {
    try {
      ...
      while (hasNextPage == true) {
        movies = await moviesRepo.getMoviesFromRemote(movieTitle: title, page: page);
        currentPage = page;
